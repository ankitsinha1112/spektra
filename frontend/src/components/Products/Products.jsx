...
const Products = () => {
    ...
    const [type, setType] = useState(location.search ? location.search.split("=")[1] : "");
    ...
    useEffect(() =>{
        ...
        setIsDisabledArray(Array(products).fill(false));
    },[products])
    const toggleIsDisabled = (index) => {
        console.log('called')
        setIsDisabledArray((prev) => {
            const newArray = [...prev];
                                ...
                                >
                                    {products?.map((product, index) => (
                                        <div key={index}>
                                         ...
                                         isDialog={isDisabledArray[index]}
                                         onClose={() => toggleIsDisabled(index)}
                                         dialogText="Done!"
                                         data={product}
                                         buttonText="Okay!"
                                         />
                                            <Product {...product} key={product._id} click={() => toggleIsDisabled(index)} />
                                        </div>
                                        ))
                                    }
                                </div>
